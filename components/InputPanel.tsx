...
export const InputPanel: React.FC<InputPanelProps> = (props) => {
  ...
  const renderStringsToCatalog = () => {
    ...
    return (
        <>
            <h2 className="text-lg font-semibold text-slate-100 mb-3 flex-shrink-0">Input (.strings, .stringsdict)</h2>
            {files.length > 0 && (
                <div className="mb-2 flex-shrink-0">
                    <input
                        ...
                        className="w-full text-sm p-2 bg-slate-800 border border-slate-600 rounded-md focus:ring-1 focus:ring-red-500 focus:outline-none placeholder:text-slate-500"
            ...
            <div className="flex-grow overflow-y-auto bg-slate-900/50 rounded-md shadow-inner border border-slate-700 p-2 space-y-2 min-h-[150px]">
                {files.length === 0 ? (
                    <div className="flex items-center justify-center h-full text-slate-400"><p>Upload files to begin.</p></div>
                ) : filteredFiles.length === 0 ? (
                    <div className="flex items-center justify-center h-full text-slate-400"><p>No files match your search.</p></div>
                ) : (
                    filteredFiles.map((file) => (
                        <div key={file.name} className="flex items-center justify-between p-2 bg-slate-800 rounded-md">
                            ...
                            <div className="flex items-center space-x-2">
                                <input type="text" value={file.langCode} onChange={(e) => handleLangCodeChange(file.name, e.target.value)} placeholder="lang" className="w-20 text-sm p-1 bg-slate-700 border border-slate-600 rounded-md focus:ring-1 focus:ring-red-500 focus:outline-none" aria-label={`Language code for ${file.name}`}/>
                                <button onClick={() => handleRemoveFile(file.name)} className="p-1 text-slate-400 hover:text-red-500 hover:bg-red-500/10 rounded-full transition-colors"><CloseIcon className="w-4 h-4" /></button>
                            </div>
                        </div>
                    ))
                )}
            ...
            <div className="flex items-center justify-between mt-4 flex-shrink-0">
                <button onClick={handleUploadClick} className="flex items-center space-x-2 px-4 py-2 text-sm font-medium text-slate-300 bg-slate-700/50 border border-slate-600 rounded-md shadow-sm hover:bg-slate-600/50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-red-500 transition-all">
  ...
  const renderCatalogToStrings = () => {
    ...
    return (
        <>
            <h2 className="text-lg font-semibold text-slate-100 mb-3 flex-shrink-0">Input (.xcstrings)</h2>
            <div className="flex-grow overflow-y-auto bg-slate-900/50 rounded-md shadow-inner border border-slate-700 p-2 space-y-2 min-h-[150px]">
                {catalogFile ? (
                     <div className="flex items-center justify-between p-2 bg-slate-800 rounded-md h-full">
                        <span className="text-sm font-mono text-slate-300 truncate pr-2" title={catalogFile.name}>{catalogFile.name}</span>
                        <button onClick={() => onCatalogFileChange(null)} className="p-1 text-slate-400 hover:text-red-500 hover:bg-red-500/10 rounded-full transition-colors"><CloseIcon className="w-4 h-4" /></button>
                    </div>
                ) : (
                    <div className="flex items-center justify-center h-full text-slate-400"><p>Upload an .xcstrings file to begin.</p></div>
                )}
            </div>
            {error && <p className="text-sm text-red-400 mt-2 flex-shrink-0">{error}</p>}
            <div className="flex items-center justify-between mt-4 flex-shrink-0">
