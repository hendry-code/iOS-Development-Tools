...
const TabButton: React.FC<{
  ...
  <button onClick={onClick} className={`flex items-center space-x-2 px-4 py-2 text-sm font-medium rounded-t-lg focus:outline-none focus:ring-2 focus:ring-inset focus:ring-red-500 transition-all duration-200 flex-shrink-0 ${
      isActive ? 'bg-slate-800/50 text-red-400 border-b-2 border-red-500' : 'text-slate-400 hover:text-slate-200'
    }`}
  >{children}</button>
);
...
const SubTabButton: React.FC<{ isActive: boolean; onClick: () => void; children: React.ReactNode; }> = ({ isActive, onClick, children }) => (
    <button onClick={onClick} className={`px-3 py-1 text-xs font-medium rounded-md focus:outline-none transition-colors duration-150 flex-shrink-0 ${
        isActive ? 'bg-red-600 text-white shadow-sm' : 'text-slate-300 bg-slate-700/50 hover:bg-slate-600/50'
      }`}>{children}</button>
);
    ...
    return (
        <div className="flex items-center justify-between p-3 bg-slate-800/60 rounded-md">
            ...
            <div className="flex items-center space-x-2">
                <button onClick={handleCopy} title="Copy" className="p-1.5 text-slate-400 hover:text-red-400 hover:bg-slate-700/50 rounded-md transition-colors">{copied ? <span className="text-xs px-1">Copied!</span> : <CopyIcon className="w-4 h-4" />}</button>
...
export const OutputPanel: React.FC<OutputPanelProps> = (props) => {
    ...
    const renderStringsToCatalog = () => {
        ...
        if (!hasOutput) {
            return <div className="flex items-center justify-center h-full text-slate-400 bg-slate-900/50 rounded-b-lg"><p>Output will appear here.</p></div>
        }

        return (
            <div className="flex flex-col h-full">
                <div className="flex flex-nowrap overflow-x-auto border-b border-slate-700 flex-shrink-0">
                ...
                <div className="flex-grow pt-3 overflow-hidden">
                    ...
                    {activeTab === OutputFormat.ANDROID && (
                        <div className="h-full flex flex-col">
                            <div className="flex flex-nowrap overflow-x-auto space-x-2 p-1.5 bg-slate-900/50 rounded-md mb-2 flex-shrink-0">
    ...
    const renderCatalogToStrings = () => {
        ...
        if (!hasOutput) {
            return <div className="flex items-center justify-center h-full text-slate-400 bg-slate-900/50 rounded-b-lg"><p>Output files will appear here.</p></div>
        }
